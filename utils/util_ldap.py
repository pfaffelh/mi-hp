...
# todo: extend to today < ausstiegsdatum
all_ldap = list(per.find({"ldap" : True}))

server = Server(ldap_host)
conn = Connection(server, user=f"cn={ldap_username},dc=de", password=ldap_password, auto_bind=True)
base_dn="dc=de"
...
def upsert_person(*, cn, attributes, object_classes = None):
    """
    Upsert by cn:
